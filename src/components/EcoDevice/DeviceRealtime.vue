              ...
              <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div class="absolute top-3 right-3">
                    ...
                    >
                      <template v-if="sensor.value === null || sensor.value === undefined">
                        ⚠️ No Sensor
                      </template>
            ...
            <div class="p-6">
              <form @submit.prevent="updateThresholds" class="space-y-6">
                <div class="grid grid-cols-4 gap-4">
                  <template v-for="(row, rowIndex) in thresholdGridRows" :key="rowIndex">
...
export default {
  ...
  computed: {
    sensorDisplayList() {
      // Compose sensor display info in firmware order
      return SENSOR_ORDER.map(s => {
        const value = this.sensorReadings[s.key];
        let isAlert = false;
    ...
    thresholdGridOrder() {
      // Always show all 12 thresholds in the firmware order
      return THRESHOLD_ORDER.filter(k => k in this.thresholds);
    },
    thresholdGridRows() {
      // Arrange as 4x4 grid, Water Level Min beside TDS Max
      // Row 1: humidity_min, humidity_max, air_temp_min, air_temp_max
      // Row 2: water_temp_min, water_temp_max, ph_min, ph_max
      // Row 3: tds_min, tds_max, water_level_min, ''
      // Row 4: '', '', '', ''
      return [
  ...
  methods: {
    ...
    updateThresholds() {
      // Only validate and save real threshold keys
      const filtered = {};
      for (const [key, val] of Object.entries(this.thresholds)) {
        if (key === "restore_defaults") continue;
        if (this.isInvalid(val)) {
          this.showToast(`❌ Invalid value for "${key.replaceAll('_', ' ')}". Please correct it.`, 'error');
          return;
        }
        filtered[key] = val;
      }

      // Validate min < max for each threshold pair
      for (const [minKey, maxKey] of THRESHOLD_PAIRS) {
        if (
          ...
          this.showToast(`❌ "${this.thresholdLabel(minKey)}" must be less than "${this.thresholdLabel(maxKey)}".`, 'error');
          return;
        }
      }
      ...
      update(ref(db, `/devices/${this.deviceId}/thresholds`), filtered)
        .then(() => {
          this.showToast('✅ Thresholds updated successfully!');
        })
        .catch(() => {
    ...
    restoreDefaults() {
      ...
      const db = getDatabase();
      const restoreRef = ref(db, `/devices/${this.deviceId}/thresholds/restore_defaults`);
      set(restoreRef, true)
        .then(() => {
          this.showToast('✅ Defaults restore requested!');
          // Do NOT clear the flag here; let the firmware set it to false after restoring
          // Optionally reload thresholds after a short delay
          setTimeout(() => this.loadData(), 2500);
        })
        .catch(() => {
          this.showToast('❌ Failed to request restore.', 'error');
        });
    },
    loadData() {
  const db = getDatabase();
