<template>
  <div class="relative">
    ...
    >
      <div
        v-if="sidebarOpen"
        class="fixed inset-0 bg-emerald-500/20 backdrop-blur-md z-10 md:hidden cursor-pointer"
        @click="closeSidebar"
    ...
    <aside :class="[
      'bg-white shadow-lg z-20 transition-all duration-300 ease-in-out flex flex-col',
      sidebarOpen ? 'translate-x-0' : '-translate-x-full',
      'md:translate-x-0 fixed md:relative inset-y-0 left-0 w-64 sm:w-60 border-r border-slate-200'
    ]">
      <!-- Sidebar Header -->
      <div class="p-3 sm:p-4 border-b border-slate-200 flex justify-between items-center bg-gradient-to-r from-emerald-50 to-teal-50 flex-shrink-0">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-base sm:text-lg font-bold text-slate-800">Menu</h2>
        </div>
        <button
          @click="closeSidebar"
          class="md:hidden text-slate-500 hover:text-slate-700 p-1.5 hover:bg-slate-200/50 rounded-lg transition-all active:scale-95"
      ...
      <nav class="p-3 flex-1 overflow-y-auto">
        <div v-for="(section, index) in navigationSections" :key="index" class="mb-5">
          <h3 class="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 px-2.5">{{ section.title }}</h3>
          <ul class="space-y-1">
            <li v-for="(route, idx) in getFilteredRoutes(section)" :key="idx">
              <router-link
                :to="route.path"
                :class="[
                  'flex items-center px-3 py-2.5 text-xs sm:text-sm rounded-lg transition-all group',
                  currentRoute === route.path 
                    ? 'bg-gradient-to-r from-emerald-100 to-teal-50 text-emerald-700 font-semibold shadow-sm border border-emerald-200/50' 
                    : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900 active:scale-[0.98]'
                ]"
                @click="closeSidebar"
              >
                <svg xmlns="http://www.w3.org/2000/svg" 
                     class="h-4 w-4 sm:h-5 sm:w-5 mr-3 flex-shrink-0 transition-transform group-hover:scale-110" 
                     :class="currentRoute === route.path ? 'text-emerald-600' : 'text-slate-500'" 
                     fill="none" 
                     viewBox="0 0 24 24" 
                     stroke="currentColor">
      ...
      <div class="flex-shrink-0 border-t border-slate-200 bg-slate-50">
...
/* Better hover states */
nav a:not(.router-link-active):hover svg {
